...
local function snapshot_inv()
	...
	for i, stack in ipairs(inv.main) do
		initial_inv[i] = {name = stack:get_name(), count = stack:get_count()}
	end
end

local function get_collected_slots()
	local inv = core.get_inventory("current_player")
	if not inv then return {} end
	local slots = {}
	for i, stack in ipairs(inv.main) do
		if not stack:is_empty() then
			local init = initial_inv[i]
			local init_count = init and init.name == stack:get_name() and init.count or 0
			if stack:get_count() > init_count then
				table.insert(slots, {idx = i, excess = stack:get_count() - init_count})
			-- Also include items that weren't in the initial slot (even if empty originally)
			elseif not init then
				table.insert(slots, {idx = i, excess = stack:get_count()})
			end
		end
	end
	return slots
...
sbots.register_bot("ItemCollector", {
		...
		do_pos = function(self, pos)
			...
			for _, obj in pairs(core.get_objects_inside_radius(pos, range)) do
				if obj:get_name() == "__builtin:item" then
					if vector.distance(pos, obj:get_pos()) < 2 then
						return false
					end
				end
			end
		...
		do_step = function(self, dtime)
			if self._deposit_phase then
				if self._deposit_phase == 1 then
					...
					self._chest_pos = cp
					self._deposit_pending = get_collected_slots()
			...
			local slots = get_collected_slots()
			if #slots >= 24 then
