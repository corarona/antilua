...
| Command | Description |
|---------|-------------|
| `/nl <sub> [args]` | Umbrella command: `select` `add` `rem` `toggle` `show` `new` `delete` `export` `import` |
...
`nlist.add(list, node [, silent])` — insert `node` into the named list (if not already present).
...
`nlist.merge(dst, src)` — union `src` into `dst` without clobbering existing entries.
...
| `place.dig` | bool | false | Dig solid blocks before placing scaffold |
...
- The waypoint GUI shows a big-map section centered on the selected waypoint
  right under its screenshot, with a **View on Map** button that opens the
  fullscreen big map centered on the waypoint. Rendered section images are
  transient and are cleared on disconnect.
