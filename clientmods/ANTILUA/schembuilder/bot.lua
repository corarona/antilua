...
if sbots and sbots.register_bot then
	...
	sbots.register_bot("SchemBuilderBot", {
		...
		do_pos = function(self, pos)
			...
			local opts = {
				batch_size = tonumber(core.settings:get("schembuilderbot.batch_size")) or 8,
				range = tonumber(core.settings:get("schembuilderbot.range")) or 4,
				cooldown = tonumber(core.settings:get("schembuilderbot.place_cooldown")) or 0.1,
				strategy_name = core.settings:get("schembuilderbot.place_strategy") or "cluster",
				filter_mode = core.settings:get("schembuilderbot.filter_mode") or "all",
				filter_list = core.settings:get("schembuilderbot.filter_list") or "schembuilder",
