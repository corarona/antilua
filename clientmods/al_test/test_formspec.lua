...
function test_formspec(T)
	...
	T.run("al_formspec.scroll_container with opts appends factor and padding", function()
		local result = core.al_formspec.scroll_container(0, 1, 9, 9, "mscroll", "vertical", { factor = 0.1, padding = 0 })
		T.assert_eq(result, "scroll_container[0,1;9,9;mscroll;vertical;0.1;0]", "scroll_container with opts")
		local noopts = core.al_formspec.scroll_container(0, 1, 9, 9, "mscroll")
		T.assert_eq(noopts, "scroll_container[0,1;9,9;mscroll;vertical]", "scroll_container without opts unchanged")
	end)

	T.run("al_formspec.item_image produces correct format", function()
		local result = core.al_formspec.item_image(0, 0, 1, 1, "default:stone")
		T.assert_eq(result, "item_image[0,0;1,1;default:stone]", "item_image format")
	end)

	T.run("al_formspec.tooltip produces correct format", function()
		local result = core.al_formspec.tooltip("btn1", "Hello [x]")
		T.assert(result:find("^tooltip%[") ~= nil, "tooltip element")
		T.assert(result:find("Hello") ~= nil, "tooltip text")
		T.assert(result:find("btn1") ~= nil, "tooltip target id")
	end)

	T.run("al_formspec.scrollbar produces correct format", function()
		local result = core.al_formspec.scrollbar(0, 1, 0.2, 5, "vertical", "mscroll", 0)
		T.assert_eq(result, "scrollbar[0,1;0.2,5;vertical;mscroll;0]", "scrollbar format")
		local default = core.al_formspec.scrollbar(0, 1, 0.2, 5, "vertical", "mscroll")
		T.assert(default:match(";0%]") ~= nil, "scrollbar defaults to value 0")
	end)
end
