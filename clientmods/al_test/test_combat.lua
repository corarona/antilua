...
function test_combat(T)
	...
	T.run("killaura.range default exists", function()
		...
		T.assert(v ~= nil)
		T.assert(tonumber(v) ~= nil, "killaura.range should be numeric")
	end)
	...
	T.run("killaura.get exists and returns defaults", function()
		...
		T.assert(type(killaura.get("range")) == "number")
		T.assert(killaura.get("range") > 0)
	end)
	...
	T.run("killaura.range accepts boundary values", function()
		local orig = core.settings:get("killaura.range")
		...
		core.settings:set("killaura.range", orig or "4.5")
	end)

	T.run("killaura.mace_fall_distance setting exists", function()
