...
void GUIKeyChangeMenu::regenerateGui(v2u32 screensize)
{
...

	{
		...
		//gui::IGUIStaticText *t =
			std::wstring text_ks = wstrgettext("Keybindings.");
		Environment->addStaticText(text_ks.c_str(),
	...
	for(size_t i = 0; i < key_settings.size(); i++)
	{
		key_setting *k = key_settings.at(i);
		{
			...
			Environment->addStaticText(k->button_name.c_str(), rect, false, true, this, -1);
		}

		{
			core::rect<s32> rect(0, 0, 100 * s, 30 * s);
			rect += topleft + v2s32(offset.X + 150 * s, offset.Y - 5 * s);
			std::wstring btn_text = wstrgettext(k->key.name());
			k->button = GUIButton::addButton(Environment, rect, m_tsrc, this, k->id, btn_text.c_str());
		}
...

	{
		...
		u32 option_w = 180 * s;
		{
			...
			std::wstring cb_text1 = wstrgettext("\"Aux1\" = climb down");
			Environment->addCheckBox(g_settings->getBool("aux1_descends"), rect, this,
					GUI_ID_CB_AUX1_DESCENDS, cb_text1.c_str());
		}
		offset += v2s32(0, 25 * s);
	}

	{
		...
		u32 option_w = 280 * s;
		{
			core::rect<s32> rect(0, 0, option_w, 30 * s);
			rect += topleft + v2s32(option_x, option_y);
			std::wstring cb_text2 = wstrgettext("Double tap \"jump\" to toggle fly");
			Environment->addCheckBox(g_settings->getBool("doubletap_jump"), rect, this,
					GUI_ID_CB_DOUBLETAP_JUMP, cb_text2.c_str());
		}
		offset += v2s32(0, 25 * s);
	}

	{
		...
		u32 option_w = 280;
		{
			core::rect<s32> rect(0, 0, option_w, 30 * s);
			rect += topleft + v2s32(option_x, option_y);
			std::wstring cb_text3 = wstrgettext("Automatic jumping");
			Environment->addCheckBox(g_settings->getBool("autojump"), rect, this,
					GUI_ID_CB_AUTOJUMP, cb_text3.c_str());
		}
		offset += v2s32(0, 25);
	}

	{
		...
		std::wstring save_text = wstrgettext("Save");
		GUIButton::addButton(Environment, rect, m_tsrc, this, GUI_ID_BACK_BUTTON, save_text.c_str());
	}
	{
		core::rect<s32> rect(0, 0, 100 * s, 30 * s);
		rect += topleft + v2s32(size.X / 2 + 5 * s, size.Y - 40 * s);
		std::wstring cancel_text = wstrgettext("Cancel");
		GUIButton::addButton(Environment, rect, m_tsrc, this, GUI_ID_ABORT_BUTTON, cancel_text.c_str());
	}
}
...
{
	if (active_key) {
		std::wstring reset_text = wstrgettext(active_key->key.name());
		active_key->button->setText(reset_text.c_str());
		active_key = nullptr;
		return false;
	}
	return true;
...
bool GUIKeyChangeMenu::OnEvent(const SEvent& event)
{
	if (event.EventType == EET_KEY_INPUT_EVENT && active_key
		...
		if (key_in_use && !this->key_used_text) {
			...
			std::wstring inuse_text = wstrgettext("Key already in use");
			this->key_used_text = Environment->addStaticText(inuse_text.c_str(),
					rect, false, true, this, -1);
		...
		{
			active_key->key = kp;
			std::wstring kp_text = wstrgettext(kp.name());
			active_key->button->setText(kp_text.c_str());
		...
		if (event.GUIEvent.EventType == gui::EGET_BUTTON_CLICKED)
		{
			switch (event.GUIEvent.Caller->getID())
			{
				...
				default:
					...
					shift_down = false;
					std::wstring press_text = wstrgettext("press key");
					active_key->button->setText(press_text.c_str());
					break;
			}
...
void GUIKeyChangeMenu::add_key(int id, const std::wstring &button_name, const std::string &setting_name)
{
	...
	k->key = getKeySetting(k->setting_name.c_str()).at(0);
...
void GUIKeyChangeMenu::init_keys()
{
	...
	this->add_key(GUI_ID_KEY_ENDERCHEST_BUTTON,     wstrgettext("Enderchest"),		  "keymap_enderchest");
