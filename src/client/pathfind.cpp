// Antilua — A* pathfinding implementation
...
{
	return v3s16(
		static_cast<s16>(std::round(pos.X)),
		static_cast<s16>(std::round(pos.Y)),
		static_cast<s16>(std::round(pos.Z)));
}
...
double check_tool_dig_time(const ToolCapabilities &toolcaps,
	const ItemGroupList &node_groups, double old_best_time)
{
	...
	for (const auto &groupcap_pair : toolcaps.groupcaps) {
		...
		if (it != node_groups.end()) {
			int level = it->second;
			auto level_it = std::find_if(groupcap.times.begin(),
				groupcap.times.end(),
				[level](const std::pair<int, float> &p) {
					return p.first == level;
				});

			if (level_it != groupcap.times.end() &&
				level_it->second < best_time)
...
double get_best_dig_time(Client *client, const ItemGroupList &groups)
{
	...
	for (u32 i = 0; i < main->getSize(); i++) {
		...
		const ToolCapabilities &toolcaps =
			stack.getToolCapabilities(client->idef());
...
static s32 get_node_cost(const v3f &pos, Client *client,
	const NodeDefManager *ndef)
{
	...
	if (!pos_ok) {
		...
		cost = static_cast<s32>(
			get_best_dig_time(client, node_def.groups) * 200);
	}
...
static bool get_node_walkable(const v3f &pos, Client *client,
	const NodeDefManager *ndef)
{
...
static s32 get_path_segment_cost(const v3f &current_pos, const v3f &from_pos,
	Client *client, const NodeDefManager *ndef)
{
	...
	if (base_cost == std::numeric_limits<s32>::max())
		return base_cost;
	cost += base_cost;

	s32 head_cost = get_node_cost(current_pos + v3f(0, 1, 0), client, ndef);
	if (head_cost == std::numeric_limits<s32>::max())
		return head_cost;
	cost += head_cost;

	if (delta.X != 0 && delta.Z != 0) {
		s32 side1_cost = get_node_cost(
			from_pos + v3f(delta.X, 0, 0), client, ndef);
		if (side1_cost == std::numeric_limits<s32>::max())
			return side1_cost;
		cost += side1_cost;

		s32 side2_cost = get_node_cost(
			from_pos + v3f(0, 0, delta.Z), client, ndef);
		if (side2_cost == std::numeric_limits<s32>::max())
			return side2_cost;
		cost += side2_cost;

		s32 side1_head_cost = get_node_cost(
			from_pos + v3f(delta.X, 1, 0), client, ndef);
		if (side1_head_cost == std::numeric_limits<s32>::max())
			return side1_head_cost;
		cost += side1_head_cost;

		s32 side2_head_cost = get_node_cost(
			from_pos + v3f(0, 1, delta.Z), client, ndef);
		if (side2_head_cost == std::numeric_limits<s32>::max())
			return side2_head_cost;
		cost += side2_head_cost;
	}

	if (delta.Y > 0) {
		for (int i = 1; i < delta.Y + 1; ++i) {
			v3f intermediate_pos_y = from_pos + v3f(0, i, 0);
			s32 c = get_node_cost(intermediate_pos_y, client, ndef);
			if (c == std::numeric_limits<s32>::max())
				return c;
			cost += c;
		}
		s32 c = get_node_cost(
			current_pos + v3f(0, 2, 0), client, ndef);
		if (c == std::numeric_limits<s32>::max())
			return c;
		cost += c;
	}
...
PathNode::PathNode(const v3f &pos, const v3f &target, Client *client,
	const NodeDefManager *ndef, PathNode *owner)
	: position(pos), target_position(target), parent(owner)
{
	PERF_TIMER("PathNode::PathNode");

	s32 current_node_a_cost = get_path_segment_cost(
		pos, owner ? owner->position : pos, client, ndef);

	h_cost = get_g_cost(pos, target);

	if (owner) {
		g_cost = owner->g_cost + get_g_cost(pos, owner->position) +
			current_node_a_cost;
	} else {
...
std::vector<PathNode> Pathfind::get_path(v3f start, v3f end, Client *client,
	const NodeDefManager *ndef, int max_depth, bool debug)
{
	auto pathfind_start_time =
	...
	std::priority_queue<PathNode *, std::vector<PathNode *>, NodeCompare>
		open_set;
	std::unordered_map<v3s16, PathNode *, v3s16_hash, v3s16_equal>
		all_nodes;

	std::set<v3s16> closed_set;

	PathNode *start_node =
		new PathNode(start, end, client, ndef, nullptr);
	...
	while (!open_set.empty() && depth < max_depth) {
		...
		for (const v3s16 &neighbor_pos : get_neighbors(current_pos)) {
			...
			s32 current_to_neighbor_segment_cost =
				get_g_cost(neighbor_f, current->position);
			s32 neighbor_a_cost = get_path_segment_cost(
				neighbor_f, current->position, client, ndef);

			if (neighbor_a_cost == std::numeric_limits<s32>::max())
				continue;

			s32 tentative_g_cost = current->g_cost +
				current_to_neighbor_segment_cost + neighbor_a_cost;

			auto it = all_nodes.find(neighbor_pos);
			if (it == all_nodes.end()) {
				PathNode *new_node = new PathNode(
					neighbor_f, end, client, ndef, current);
				...
				if (tentative_g_cost < existing_node->g_cost) {
					...
					existing_node->f_cost =
						existing_node->g_cost +
						existing_node->h_cost;
	...
	if (!result_node) {
		...
		if (debug) {
			errorstream << "Path not found to exact target. "
				   << "Falling back to best node found."
				   << std::endl;
		}
	}
	...
	if (debug) {
		auto pathfind_end_time =
			std::chrono::high_resolution_clock::now();
		auto total_duration_ms =
			std::chrono::duration_cast<
				std::chrono::milliseconds>(
				pathfind_end_time - pathfind_start_time)
				.count();

		errorstream << "========== Pathfind End ==========" << std::endl;
		errorstream << "Pathfinding from (" << start.X << ","
		...
		errorstream << "Nodes checked from open set: "
			   << nodes_checked << std::endl;
		errorstream << "Max depth reached (iterations): " << depth
			   << " / " << max_depth << std::endl;
		errorstream << "Path length: " << path.size() << " nodes"
			   << std::endl;
		errorstream << "------------------------------------"
			   << std::endl;
			...
			if (stats.call_count > 0) {
				...
				double avg_us =
					std::chrono::duration_cast<
						...
						stats.total_time)
						.count() /
					(double)stats.call_count / 1000.0;
				errorstream << name << ":" << std::endl;
				errorstream << "    Calls: " << stats.call_count
					   << std::endl;
				errorstream << "    Total Time: " << total_ms
					   << " ms" << std::endl;
				errorstream << "    Avg. Time: " << avg_us
					   << " us/call" << std::endl;
			}
		}
		errorstream << "=================================="
